...
public class PlaidManager {
    ...
    static let groupIdentifier = "FinVine"
...
extension PlaidManager {
    ...
    public var info: PlaidInfo {
        ...
        print("public key is"+(info["environment"] ?? "something wrong"))
        let key = info["publicKey"] ?? "bad public"
        let clientID = info["clientID"] ?? "bad client"
        let secret = info["secret"] ?? "bad secret"
        let env = info["environment"] ?? "bad env"
        let environment = PlaidInfo.Environment(rawValue: env) ?? nil
        if (environment == nil){
            print("env null")
        }
        
        
        //print("result is "+myDictionary["shortterm"]!)
